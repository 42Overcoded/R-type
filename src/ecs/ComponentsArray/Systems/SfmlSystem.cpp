...
void SfmlSystem::load_texture(registry &r)
{
    ...
    try {
        ...
        if (!wall.loadFromFile(PATH_TO_ASSETS + "wall2.png"))
            throw std::runtime_error("Cannot load wall texture");
        if (!wall2.loadFromFile(PATH_TO_ASSETS + "wall3.png"))
            throw std::runtime_error("Cannot load wall2 texture");
        if (!asteroid.loadFromFile(PATH_TO_ASSETS + "asteroid.png"))
            throw std::runtime_error("Cannot load asteroid texture");
